...
class agent:
    ...
    def next_move(self, game_state, player_state):
        ...
        # ore_predicted_damage update ore_predicted_damage = calc_ore_blocks
        blast_predictions = self.get_blast_locations(game_state)
        ...
        next_djikstra_location = self.djikstra_to_location (game_state, player_state)
        ...
        # Established escape protocol takes precedence over bombing
        if len(self.escape_route) != 0:
            if not (action in self.escape_route):
                print('Escaping...')
                print(self.escape_route)
                action = random.choice(self.escape_route)
                #TODO Stretch - Test escape protocols more extensively
                self.escape_route = []

        elif (next_block == 'ob') or (next_block == 'sb'):
            if(player_state.ammo > 0):
                # Don't bomb blocks to chase enemy, not worth
                if((self.current_target[0], self.current_target[1]) != (opponent_location[0], opponent_location[1])):
                    # Before:
                    safe_actions = self.escape_if_bombed(game_state, self.location[0], self.location[1])
                    if len(safe_actions) != 0:
                        print('Bombing obstacle...')
                        action = 'b'
                        self.escape_route = safe_actions
                        ...
                        # If there are no safe actions, it's because you're trapped.
                        # self.suspended_treasure.append ((self.current_target[0], self.current_target[1], 35))
                        # TODO - Stretch: Figure out escape/wait protocols
                        # TODO Stretch - don't just sit trapped, move to safest spot
                        print('Trapped :(((')

        elif next_block == 'ib':
            print('WARN unexpected metal at: ')
            print(next_djikstra_location)
        elif (next_block == 1) or (next_block == 0) or (next_block == 'b'):  # Note bomb masks player
            if (next_djikstra_location[0], next_djikstra_location[1]) == (opponent_location):
                # Enemy
                # Murder = adhoc decision during pursuit, process after djikstra selection
                # TODO - advanced murder (2 space, including both distance and doubling up)
                # TODO stretch -  expert murder (3 space)
                if self.is_opponent_surrounded (game_state):
                    print('Surrounded, time for murder...')
                    action = 'b'
            #else:
                # Just a bomb
    ...
    def djikstra_to_location(self, game_state, player_state):
            ...
            for djikstra_tile in djikstra_tiles:
                for relevant_blast in relevant_blasts:
                    ...
                    if ((djikstra_tile[0], djikstra_tile[1]) == (relevant_blast[0], relevant_blast[1])):
                        ...
                        if abs(djikstra_tile[2]-relevant_blast[2]) < 5:
        ...
        # For singled ore valuation calcs, for box valuation calcs
        max_walking_cost = 0
        for djikstra_tile in djikstra_tiles:
            if (djikstra_tile[2] < 38) and (djikstra_tile[2]>max_walking_cost):
                max_walking_cost = djikstra_tile[2]

        # PRIORITY LOWEST - GET PICKUPS / SHED BOMBS / ZONE ENEMY (highest to lowest)
        ...
        opponent_location = self.get_opponent_location (game_state)
        # get the nearest viable pickup (check you can afford it)
        viable_pickups = []
        if len (pickup_tiles) > 0:
            viable_cost = (player_state.ammo*38) + max_walking_cost
            for pickup_tile in pickup_tiles:
                if pickup_tile[2] < viable_cost:
                    viable_pickups.append(pickup_tile)
        if len(viable_pickups) > 0:
            nearest_pickup = viable_pickups[0]
            for viable_pickup in viable_pickups:
                if (nearest_pickup[2] > viable_pickup[2]):
                    nearest_pickup = viable_pickup
            target_tile = nearest_pickup
            print('Pickup...')

        # Hunt boxes if excess of bombs - if higher than (6 for murder and chasing + (net ore number))
        # TODO Stretch - in bomb shedding ignore inaccessible ores for the count. (equals or higher max_value indicates unreachable)
        elif player_state.ammo > (6 + len(game_state.ore_blocks)):
            # Get nearest box
            if len(game_state.soft_blocks) != 0:
                block_location = game_state.soft_blocks[0]
                closest_box = self.get_tile(djikstra_tiles, block_location[0], block_location[1])
                # Cycle through soft blocks
                for soft_block in game_state.soft_blocks:
                    # Get corresponding djikstra cost
                    new_block = self.get_tile(djikstra_tiles, soft_block[0], soft_block[1])
                    # Compare with record holder
                    if closest_box[2] > new_block[2]:
                        closest_box = new_block
                # Set target to record holder
                target_tile = closest_box
                print ('Shedding...')
        ...
        # Find ore cost
        # One hit ore found
        vulnerable_ores = [] #Contains location[0], [1], integrity, use to find the djikstra cost.

        # for this_ore in self.ore_tracked:
        for this_ore in self.ore_predicted_damage:
            if this_ore[2] == 1:
                vulnerable_ores.append(this_ore)

        vulnerable_ores_cost = []
        for vulnerable_ore in vulnerable_ores:
            vulnerable_ore_cost = self.get_tile(djikstra_tiles, vulnerable_ore[0], vulnerable_ore[1])
            vulnerable_ores_cost.append(vulnerable_ore_cost)

        # Find the cheapest ore
        # vulnerable_ores_cost = [(x, y, djikstra cost)]
        if len(vulnerable_ores_cost) > 0:
            cheapest_vulnerable_ore = vulnerable_ores_cost[0]
            for vulnerable_ore_cost in vulnerable_ores_cost:
                if cheapest_vulnerable_ore[2] > vulnerable_ore_cost[2]:
                    cheapest_vulnerable_ore = vulnerable_ore_cost
            # It's worth it if it can be afforded (ignore previous check about walk distance accessible
            if (cheapest_vulnerable_ore[2] <= ((player_state.ammo)*38 + max_walking_cost)):
                # So target it next.
                target_tile = cheapest_vulnerable_ore
                print ('Chasing ore...')
                print (cheapest_vulnerable_ore)

        # TODO Stretch - don't enter alleys if opponent is nearby, priority slightly lower than avoiding blasts
        ...
        while ((next_tile[0], next_tile[1]) != (self.location[0], self.location[1])):
            ...
            if (next_tile[0], next_tile[1]) == (action_tile[0], action_tile[1]):
                next_tile = self.get_tile(djikstra_tiles, self.location[0], self.location[1])
                action_tile = next_tile
    ...
    # My own method to check if a point is blocked, I don't trust the game functions
    ...
    # Figure out the escape routes for this point if it's bombed, return array of actions (l, r, u, d)
    def escape_if_bombed(self, game_state, x, y):
        actions = []
        # Right
        if self.is_blocked(game_state, x+1, y) == False:
            # Check corners
            if (self.is_blocked (game_state, x+1, y+1) == False) or (self.is_blocked (game_state, x+1, y-1) == False):
                actions.append('r')
            # Check 2 spaces
            elif (self.is_blocked (game_state, x+2, y) == False):
                # Check 2 space corners
                if (self.is_blocked (game_state, x + 2, y + 1) == False) or (self.is_blocked (game_state, x + 2, y - 1) == False):
                    actions.append ('r')
        ...
        # Up
        if self.is_blocked(game_state, x, y+1) == False:
            # Check corners
            if (self.is_blocked (game_state, x+1, y+1) == False) or (self.is_blocked (game_state, x-1, y+1) == False):
                actions.append('u')
            # Check 2 spaces
            elif (self.is_blocked (game_state, x, y+2) == False):
                # Check 2 space corners
                if (self.is_blocked (game_state, x-1, y+2) == False) or (self.is_blocked (game_state, x+1, y+2) == False):
                    actions.append ('u')
        ...
        for next_location in valid_next_locations:
            ...
            if (next_tile != 'tile not found'):
                ...
                if self.get_tile (new_obstacles, next_location[0], next_location[1]) != 'tile not found':
                    # Overriding obstacle #TODO Stretch - rename to reflect override is solely explosion driven
                    # Approximate explosion as 2* ore tiles
                    new_tile = (next_location[0], next_location[1], minPQ_smallest[2] + (38*6))
                    ...
                    if block_type == 'ib':
                        ...
                        bombed_ore = self.get_tile (self.ore_tracked, next_location[0], next_location[1])
                        # Just in case my tracking is off
                        if bombed_ore != 'tile not found':
                            bombs_needed = bombed_ore[2]
                            new_tile = (next_location[0], next_location[1], minPQ_smallest[2] + (38 * bombs_needed))
                        else:
                            # game state contains ore, but not tracked - likely due to premature removal. 1 Step added to account for lag
                            # print ('djikstra ore tracking mismatch')
                            # print (self.ore_tracked)
                            # print ('ore located')
                            # print (next_location[0], next_location[1])
                            new_tile = (next_location[0], next_location[1], minPQ_smallest[2] + 2)
                else:
    ...
    # Also loads self.ore_predicted_damage = calc_ore_blocks
    def get_blast_locations(self, game_state):
        ...
        for bomb_tracked in ordered_bombs_tracked:
            ...
            # remove calc_ore_blocks if they have no more integrity
            for calc_ore_block in calc_ore_blocks:
                if calc_ore_block[2] == 0:
                    calc_ore_blocks.remove(calc_ore_block)
        ...
        # Load predictions to self.ore_predicted_damage
        self.ore_predicted_damage = calc_ore_blocks
    ...
    def update_bombs_tracked(self, game_state):
        ...
        # Check if there are ores that we missed in the state
        ...
        # Check if we are tracking ores that aren't there
        for this_ore in self.ore_tracked:
            if (self.get_tile (game_state.ore_blocks, this_ore[0], this_ore[1]) == 'tile not found'):
                # this_ore that's tracked isn't in the game. Delete it.
                self.ore_tracked.remove(this_ore)
        ...
        for bomb_location in bomb_locations:
            ...
            if already_tracked == False:
                ...
                new_bomb = (bomb_location[0], bomb_location[1], 34) #34 ticks till explode, 1 for pre-calculating bomb deletion
